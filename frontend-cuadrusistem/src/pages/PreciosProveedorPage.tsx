...
export function PreciosProveedorPage() {
  const [view, setView] = useState<"categorias" | "proveedores" | "productos">(
    "categorias",
  );
    ...
    <div className="max-w-md mx-auto min-h-screen bg-gray-50 pb-20">
      ...
      <div className="sticky top-0 bg-white shadow-sm p-4 z-10 flex items-center gap-3">
        ...
        <h2 className="text-xl font-bold text-gray-800">
          ...
          {view === "proveedores" &&
            `Proveedores: ${CATEGORIAS_COCINA.find((c) => c.id === selectedCat)?.nombre}`}
        ...
        {view === "categorias" && (
          <div className="grid grid-cols-2 gap-4">
            {CATEGORIAS_COCINA.map((cat) => (
              ...
              >
                <span
                  className={`text-4xl mb-3 p-4 rounded-full ${cat.color} group-hover:scale-110 transition-transform`}
                >
                  {cat.icon}
                </span>
            ...
            {filteredProviders.length > 0 ? (
                ...
                >
                  <div>
                    ...
                    <span className="text-sm text-gray-500">
                      {
                        prov.productos.filter(
                          (p) => p.categoria === selectedCat,
                        ).length
                      }{" "}
              ...
              <div className="text-center py-10">
                ...
                <p className="text-gray-500 font-medium">
                  No hay proveedores registrados para esta categoría.
                </p>
              </div>
            )}
          </div>
        ...
        {view === "productos" && (
          <div className="space-y-4">
            {filteredProducts.map((prod) => {
              const precioIva =
                prod.precioNetoUnidad * (1 + prod.ivaPorcentaje / 100);
              const precioCajaNeto =
                prod.precioNetoUnidad * prod.unidadesPorEnvase;
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="flex justify-between items-end border-b border-gray-50 pb-3">
                      ...
                      <div className="text-right">
                        <span className="text-xs text-gray-400 block italic">
                          Neto: {formatCurrency(prod.precioNetoUnidad)}
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="bg-gray-50 rounded-xl p-4 flex justify-between items-center">
                        <div>
                          <span className="block text-xs font-bold text-gray-500 uppercase">
                            Total{" "}
                            {prod.presentacion.includes("CJA")
                              ? "Caja"
                              : "Envase"}
                          </span>
                          <span className="text-xl font-bold text-blue-700">
                            {formatCurrency(precioCajaTotal)}
                          </span>
                        </div>
                        <div className="text-right text-xs text-gray-500 font-medium">
                          <span>{prod.unidadesPorEnvase} unidades</span>
                          <span className="block">
                            Neto: {formatCurrency(precioCajaNeto)}
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-100 p-2 rounded-lg">
                      ...
                      <p className="text-[10px] text-yellow-800 leading-tight">
                        Compara este precio con el del mercado actual. Si es
                        menor a{" "}
                        <span className="font-bold">
                          {formatCurrency(precioIva * 1.1)}
                        </span>
                        , ¡es una buena oferta!
                      </p>
                    </div>
